...
SetEnvProxy = function(value, proxy_closure)
    AssertFunction(value, "value", 1)
    AssertFunction(proxy_closure, "proxy_closure", 1)
    local first_name, first_upvalue = debug.getupvalue(proxy_closure, 1)
    AssertTable(first_upvalue, "proxy_closure's first upvalue", 1)
    for i = 1, 65536 do
...
Space = function(instance, parent)
    AssertTable(instance, "instance", 1)
    AssertTypes(parent, { "table", "nil" }, "parent", 1)
    ...
    local handle_keyword_newindex = function(key)
        if key == "this" then
            error("Cannot rewrite 'this'", 3) -- 禁止重写this
        end
        if key == "super" then
            error("Cannot rewrite 'super'", 3) -- 禁止重写super
        end
    end
